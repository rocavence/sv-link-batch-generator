...
def validate_result_item(result):
    """
    Validate individual result item structure
    
    Args:
        result (dict): Single result item
    ...
    required_fields = ['original', 'short', 'success']
    ...
    if not isinstance(result['short'], str):
        return False
...
if __name__ == "__main__":
    ...
    test_results = [
        {
            'original': 'https://streetvoice.com/artist1/',
            'short': 'https://sv.link/abc123',
            'success': True
        },
        {
            'original': 'https://streetvoice.com/artist2/',
            'short': 'API Key 無效',
            'success': False
        }
    ]
    ...
    # Decode and print CSV content for testing
    if result['statusCode'] == 200:
        response_body = json.loads(result['body'])
        csv_content = base64.b64decode(response_body['content']).decode('utf-8-sig')
        print("\n=== CSV Content ===")
        print(csv_content)